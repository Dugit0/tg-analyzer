...
required_fields_message = ['date', 'date_unixtime', 'from', 'from_id', 'id', 'text', 'text_entities', 'type']
optional_fields_message = ['author', 'contact_information', 'duration_seconds', 'edited', 'edited_unixtime', 'file', 'file_name', 'forwarded_from', 'game_description', 'game_link', 'game_title', 'height', 'inline_bot_buttons', 'location_information', 'media_type', 'mime_type', 'performer', 'photo', 'poll', 'reply_to_message_id', 'reply_to_peer_id', 'saved_from', 'self_destruct_period_seconds', 'sticker_emoji', 'thumbnail', 'title', 'via_bot', 'width']
required_fields_service = ['action', 'actor', 'actor_id', 'date', 'date_unixtime', 'id', 'text', 'text_entities', 'type']
optional_fields_service = ['boosts', 'discard_reason', 'duration', 'duration_seconds', 'game_message_id', 'height', 'inviter', 'members', 'message_id', 'new_icon_emoji_id', 'new_title', 'photo', 'score', 'title', 'width']
...
class Chat():
    """
    Chat object consists of telegram's chat field's + an array of Message objects.
    """
    name = None
    id = None
    type = None
    ...
    def __init__(self, message, chat, login):
        ...
        if message["type"] == "service":
            ...
            if message["action"] == "phone_call":
                self.duration = message["duration_seconds"] if "duration_seconds" in message.keys() else 0
                self.type = "single_call"
            else:
                self.duration = message["duration"] if "duration" in message.keys() else 0
            ...
            if tmp_simple_text:
            ...
            elif "media_type" in message.keys() and message["media_type"] in ["sticker",  #sticker message
                ...
                self.type = "poll"
            elif "contact_information" in message.keys():   #maybe for future
                self.type = "contact"
            elif "location_information" in message.keys():   #maybe for future
                self.type = "location"
            elif "via_bot" in message.keys():   #using a bot
                self.type = "bot_usage"
            elif "game_title" in message.keys() and (place := game_finder(login, message)):   #competition tables
                self.type = "game"
...
def start_api(login):
    ...
    for chat in old_chats:
        ...
        for message in chat['messages']:
            if message["type"] == "service" and message["action"] != "phone_call" and message["action"] != "group_call":
                continue
            tmp_mes = Message(message, chat, login)
            messages.append(tmp_mes)
        tmp_chat.messages = messages
        ret_chats.append(tmp_chat)
    return ret_chats
